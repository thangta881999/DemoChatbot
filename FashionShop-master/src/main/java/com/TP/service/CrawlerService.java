...
//          	Link truy cập danh mục con
          	String linkCategory =category.select("a[href").first().attr("href");
          	   ...
          	   while(true)
          	   {
          		   Document yameCategory = Jsoup.connect("https://yame.vn"+linkCategory+"&page="+numberPage).timeout(45000).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36")
                             .get();
...
//          		   lấy danh sách sản phẩm trong danh mục
              	  ...
              	  for(Element product : products)
              	  {
              		  ...
              		  Document yameProduct = Jsoup.connect("https://yame.vn"+linkProductByCategogy).timeout(45000).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36")
              				  .get();
...
//              			  Lấy màu + size + số lượng
              			  String mauSize=elVariant.select("td").get(1).text();
              			  String mS[]= mauSize.split(",");
              			  String mau= mS[0];
              			  String size = mS[1];
              			  String soluong=elVariant.select("td").get(2).text();
